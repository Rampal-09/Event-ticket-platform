...
const ScannerPage = () => {
    ...
    const fetchStats = async () => {
        if (!eventId) return;
        try {
            const data = await eventService.getScannerStats(eventId);
            setStats(data);
            setScannedAttendees(data.recentAttendees.map(a => ({
                id: a.id,
                name: a.buyerName,
                ticketId: a.qrPayload.split(':')[1],
                time: new Date(a.purchasedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
                status: 'Verified'
            })));
        } catch (err) {
    ...
    const handleNewScan = (result) => {
        // Refresh stats and list after a successful scan
        fetchStats();
    };

    if (isLoading) {
        return <div className="min-h-screen bg-[#0A0A0F] text-white flex items-center justify-center font-black uppercase tracking-widest">Initializing Scanner...</div>;
    }

    if (!stats) {
        return <div className="min-h-screen bg-[#0A0A0F] text-white flex flex-col items-center justify-center gap-4">
            <p className="font-bold text-gray-400">Event Not Found or Access Denied</p>
            <button onClick={() => navigate('/organizer')} className="text-indigo-400 font-black uppercase text-xs tracking-widest">Back to Dashboard</button>
        </div>;
    }
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mt-0.5">{stats.title}</p>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div className="lg:col-span-12 xl:col-span-8 space-y-8">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                            ...
                            <EventStatsCard
                                label="Fill Rate"
                                value={`${stats.totalTickets > 0 ? ((stats.ticketsSold / stats.totalTickets) * 100).toFixed(1) : 0}%`}
                                trend={-8.1}
                                color="amber"
